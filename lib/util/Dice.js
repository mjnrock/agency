...
var Dice = exports.Dice = {
	...
	ratio: function ratio(min, max) {
		return (Dice.random(min, max) - min) / (max - min);
	},
	rate: function rate(min, max) {
		var threshold = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0.500;
  ...
  */
	chance: function chance(min, max) {
		...
		return Math.round(undefined.ratio(min, max) * scalar) / scalar <= threshold;
	},
