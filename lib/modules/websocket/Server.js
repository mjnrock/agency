...
var Server = exports.Server = function (_Network) {
    ...
    function Server(wss) {
        ...
        var alter = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
        var opts = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};

        _classCallCheck(this, Server);

        var _this = _possibleConstructorReturn(this, (Server.__proto__ || Object.getPrototypeOf(Server)).call(this, state, alter));
    ...
    _createClass(Server, [{
        ...
        value: function _bind(wss, app) {
            ...
            wss.on("listening", function () {
                return _this2.emit(Server.Signal.LISTENING);
            });
            wss.on("close", function () {
                return _this2.emit(Server.Signal.CLOSE);
            });
            wss.on("connection", function (client) {
                return _this2.emit(Server.Signal.CONNECTION, client);
            });
            wss.on("headers", function (headers, req) {
                return _this2.emit(Server.Signal.HEADERS, headers, req);
            });
            ...
            app.ws("/", function (client, req) {
                client.on("message", function (packet) {
                    try {
                        ...
                        _this2.emit(Server.Signal.Client.MESSAGE, msg, client, req);
                    } catch (e) {
                        _this2.emit(Server.Signal.Client.MESSAGE_ERROR, e, packet, client, req);
                    }
                });
                client.on("close", function (code, reason) {
                    return _this2.emit(Server.Signal.Client.DISCONNECT, code, reason);
                });
            });
        }
    }, {
        ...
        value: function sendToClient(client, type) {
            ...
            if (typeof this._packer === "function") {
                if (_Message2.default.ConformsBasic(type)) {
                    ...
                    msg = (_packer = this._packer).call.apply(_packer, [this, type.type].concat(_toConsumableArray(type.data)));
                } else {
                    var _packer2;

                    msg = (_packer2 = this._packer).call.apply(_packer2, [this, type].concat(payload));
                }
            } else {
                msg = [type, payload];
            }
