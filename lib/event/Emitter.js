        ...
        get: function get() {
            ...
            return {
                emit: async function emit(event) {
                    ...
                    try {
                        for (var _iterator6 = _this.__subscribers[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
                            var subscriber = _step6.value;

                            if (typeof subscriber === "function") {
                                subscriber.call.apply(subscriber, [payload, event].concat(args));
                            } else if (subscriber instanceof Emitter) {
                                var _subscriber$$$_handle;

                                (_subscriber$$$_handle = subscriber.$._handle).call.apply(_subscriber$$$_handle, [payload, event].concat(args));
                            }
                        }
                    } catch (err) {
                 ...
                 */
                _handle: async function _handle(event) {
                    var payload = this;
                    ...
                    if (payload.provenance.has(_this) === false) {
                        ...
                        if (typeof _this.__filter === "function" && (_this$__filter = _this.__filter).call.apply(_this$__filter, [payload, event].concat(args)) === true) {
                            var _this$__relay;
                            ...
                            try {
                                for (var _iterator7 = receivers[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
                                    var receiver = _step7.value;

                                    if (typeof receiver === "function") {
                                        receiver.call.apply(receiver, [payload, event].concat(args));
                                    }
                                }
                            } catch (err) {
                            ...
                            var handlers = _this.__handlers[event] || [];
                            ...
                            if (typeof _this.__relay === "function" && (_this$__relay = _this.__relay).call.apply(_this$__relay, [payload, event].concat(args)) === true) {
                                var _this$$$emit;

                                (_this$$$emit = _this.$.emit).call.apply(_this$$$emit, [payload, event].concat(args));
                            }
                        }
